...
int fazerPergunta(tp_baralho *uni, tp_pergunta *banco_completo, int nmr_questoes, int unidade, char *nome_jogador){
    ...
    int indice_resposta_jogador = resposta - 1;

    if(indice_resposta_jogador == questao.resposta){ // Se a resposta for correta
        printf("\nPARABENS! Resposta correta.\n");
        salvarHistoricoResposta(nome_jogador, questao, indice_resposta_jogador, "Acertou");
        ...
        salvarHistoricoResposta(nome_jogador, questao, indice_resposta_jogador, "Errou");
    }
    return 0;
}
