...
void preparandoPergunta(tp_jogador *j, tp_baralho *uni1, tp_baralho *uni2, tp_baralho *uni3, tp_pergunta *banco_completo, int nmr_questoes){
    ...
    case 1: // Caso seja a unidade 1.
        resposta = fazerPergunta(j, uni1, banco_completo, nmr_questoes, 1);
        break;
    case 2: // Caso seja a unidade 2.
        resposta = fazerPergunta(j, uni2, banco_completo, nmr_questoes, 2);
        break;
    case 3: // Caso seja a unidade 3.
        resposta = fazerPergunta(j, uni3, banco_completo, nmr_questoes, 3);
        break;
...
int fazerPergunta(tp_jogador *j, tp_baralho *uni, tp_pergunta *banco_completo, int nmr_questoes, int unidade){
    ...
    if(resposta-1 == questao.resposta){ // Se a resposta for correta
        ...
        // Registra o id da pergunta acertada no histórico do jogador.
        if(j->qtd_certas < MAX_PERG_JOGADOR){
            j->id_certas[j->qtd_certas] = questao.id;
            j->qtd_certas++;
        }
        ...
        // Registra o id da pergunta errada no histórico do jogador.
        if(j->qtd_erradas < MAX_PERG_JOGADOR){
            j->id_erradas[j->qtd_erradas] = questao.id;
            j->qtd_erradas++;
        }
    }
    return 0;
