...
#include "punicao.h"
#include "vencedor.h"
#include "ranking.h" // <-- Novo include adicionado
...
int main() {
    ...
    for(int i = 0; i < MAX_PERGUNTAS; i++) {
        if(baralho[i].unidade != 0) {
            baralho[i].id = i; // <-- NOVO: id fixo da pergunta, usado pelo ranking
            num_questions++;
        }
    }
    ...
    while(!filaVazia(&fila)) {
        ...
        salvar_pontuacao(&j); // <-- NOVO: grava o resultado do jogador em disco
    }

    exibir_ranking(baralho, MAX_PERGUNTAS); // <-- NOVO: lê do disco e mostra o ranking ordenado
